...
resource "google_access_context_manager_service_perimeter_egress_policy" "egress_policies" {
  ...
  egress_from {
    ...
    source_restriction = lookup(each.value["from"], "source_restriction", null) != null ? lookup(each.value["from"], "source_restriction", null) : each.value["from"]["sources"] != {} ? "SOURCE_RESTRICTION_ENABLED" : null
  }
  egress_to {
    resources          = each.value["to"]["resources"]
